...
def eval_product_detection(img_dir, test_imgs, annotations, iou_threshold, coco, load_classifier_index, gln_state, dihe_state):
    '''
    Evaluate product detection performance.
    '''
...
@click.option('--load-classifier-index', type=click.Path(), help='Load pre-embedded images from given path instead of calculating the embedding on the fly')
@click.option('--plano-idx', type=int, help='Visualize image with given index instead of choosing randomly')
@click.argument('gln-state',
    type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True)
)
@click.argument('dihe-state',
...
def rebuild_scene(img_dir, test_imgs, test_annotations, planograms, datatype, load_classifier_index, plano_idx, gln_state, dihe_state):
    '''
    Rebuild scene from product detections.

    Runs a test image through GLN and DIHE to generate product detections,
    and then plots the test image and a "rebuilt" image generated by pasting detected product images
    at the locations of the detections.
    '''
    if datatype == 'gp':
...
@click.option('--datatype', type=click.Choice(('gp', 'internal')), default='gp', show_default=True,
    help='Dataset type; gp for Grocery Product, internal for our internal dataset')
@click.option('--load-classifier-index', type=click.Path(), help='Load pre-embedded images from given path instead of calculating the embedding on the fly')
@click.option('--verbose/--no-verbose', default=False, show_default=True, help='Print more information')
@click.argument('gln-state',
    type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True)
)
@click.argument('dihe-state',
...
def eval_planograms(img_dir, test_imgs, test_annotations, planograms, datatype, load_classifier_index, verbose, gln_state, dihe_state):
    '''
    Evaluate planogram compliance evaluation.
    '''
    if datatype == 'gp':
        planoset = datautils.PlanogramTestSet(test_imgs, test_annotations, planograms)
        sampleset = datautils.GroceryProductsDataset(img_dir, include_annotations=True)
